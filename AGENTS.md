...
- Neovim uses one package definition, `lib/neovim.nix`, for workstations, ix, and the portable command. Nix owns plugins, grammars, and matched queries; `lz.n` owns lazy loading. Plugin pins live in `dots/nvim/pack-sources.json`; update one with `just nvim-update <plugin> [revision]` and rebuild. Never add runtime installs or seed writable plugin copies. Keep the query-inheritance build check: missing shared queries silently break highlighting. See `dots/nvim/doc/config.txt` for editor behavior.
...
Neovim's package carries its plugins and curated parsers in the system closure. ix uses `nvimCurated = true` in the shared user-config builder; keep that reduced grammar set and the small CLI tool list above. Startup needs neither plugin downloads nor mutable copies under `/root`. Workstations use the full grammar set. The portable command uses the same package with the repository config supplied directly from the store.
