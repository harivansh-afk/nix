...
- The monitor is dark whenever the kiosk is not running - the console is never shown. The DRM connector is forced off (`echo off > /sys/class/drm/card1-HDMI-A-1/status`, HDMI link down, panel in standby) at boot and by `ExecStopPost` after every cast; `ExecStartPre` re-probes it (`echo detect`) before cage starts, since a force is sticky and wlroots cannot undo it. A VT blank was tried first and does not reach DPMS on this NVIDIA fbcon. `spark-display status` reports the connector's driven state as `monitor:`; if the box is ever dark and unmanaged, the `echo detect` above is the manual recovery.
