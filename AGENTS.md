...
- The monitor is dark whenever the kiosk is not running - the console is never shown. tty1 is blanked (VT blank -> fbcon DPMS off) at boot and by `ExecStopPost` after every cast, with `consoleblank=15` as the backstop; cage relights the output on its own when a cast starts. `spark-display status` reports the connector's DPMS state as `monitor:`.
