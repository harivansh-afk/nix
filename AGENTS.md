...
- The `org.nixos.spark-cast` agent supervises: `on` touches `~/.local/state/spark-cast/on`, and a 5s convergence loop keeps DeskPad running, the virtual display at native res, `sunshine.conf`'s `output_name` synced to the display's CGDirectDisplayID (it re-rolls on every DeskPad relaunch and resolution change; displayplacer's contextual id equals the id Sunshine logs), the mac's LAN IP published to spark's `/var/lib/spark-cast/host`, and the kiosk up. Kill any piece and it self-heals within a tick. `off` removes the state file; the loop tears down.
