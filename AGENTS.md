...
- spark: `hosts/spark/services/moonlight.nix` - moonlight-qt fullscreen in a cage kiosk on tty1 (`cage-tty1.service`, on-demand, boot stays multi-user). Decode is NVDEC via NVIDIA's VAAPI driver on wayland. A polkit rule scopes start/stop of that one unit to the primary user over ssh. The unit stops with SIGKILL by design: cage+moonlight never exit on SIGTERM here (every observed stop rode the timeout into SIGKILL anyway) and the kiosk is stateless, so an honest instant kill beats a 5s corpse window that races restarts for the DRM device. Kiosk logs are attributed to the user session, not the unit: `journalctl _UID=$(id -u moonlight)`, not `-u cage-tty1`.
- The monitor is dark whenever the kiosk is not running - the console is never shown. The DRM connector is forced off (`echo off > /sys/class/drm/card1-HDMI-A-1/status`, HDMI link down, panel in standby) at boot and by `ExecStopPost` after every cast; `ExecStartPre` forces it back on (`echo on`) before cage starts. Forcing is sticky both ways and wlroots cannot undo it; `echo detect` is never used because nvidia-drm's reprobe after a force-off reads disconnected with the monitor attached (cage then runs with zero outputs and moonlight streams into a placeholder). A VT blank was tried first and does not reach DPMS on this NVIDIA fbcon. `spark-display status` reports the connector's driven state as `monitor:`; if the box is ever dark and unmanaged, `echo on > /sys/class/drm/card1-HDMI-A-1/status` is the manual recovery.
- The `org.nixos.spark-cast` agent supervises: `on` touches `~/.local/state/spark-cast/on`, and a convergence loop keeps DeskPad running, the virtual display at native res, the mac's LAN IP published to spark's `/var/lib/spark-cast/host`, the kiosk up, and `sunshine.conf`'s `output_name` synced to the display's CGDirectDisplayID (it re-rolls on every DeskPad relaunch and resolution change; displayplacer's contextual id equals the id Sunshine logs). One fix per pass; each fix waits (bounded) for its own postcondition and the loop re-runs ~0.3s after acting, so a cold start walks the ladder in a few seconds, while steady state ticks every 5s. The kiosk deliberately starts before sunshine so the panel's HDMI wake and moonlight's 1s retry overlap sunshine bring-up (safe: the gate keeps sunshine down until the conf holds the fresh id). Kill any piece and it self-heals within a tick. `off` removes the state file; the loop tears down.
- The loop only ever touches DeskPad's display, found by its persistent screen UUID (learned once via DeskPad's `s1` serial, pinned in `~/.local/state/spark-cast/display-uuid`); real monitors plugged into the mac are ignored.
- Sunshine is gated on `~/.local/state/spark-cast/sunshine-on` (launchd PathState, managed by the loop): it serves only while the cast is up and the conf's display id is current. On a stale or missing id Sunshine silently captures the *real* desktop (upstream falls back to `CGMainDisplayID`), so the loop kills it before the virtual display goes away or re-rolls and only raises it after rewriting the conf. Side effect: no Sunshine ports listen while the cast is off.
- The stream target is the published LAN IP and nothing else - no fallback path. Off-LAN the kiosk just retries until the mac is reachable (mDNS is blocked on the office network, and the tailscale path costs ~30% CPU in wireguard-go on the mac; both were tried and rejected).
- `~/.config/sunshine/sunshine.conf` is owned by the convergence loop, not nix. Besides `output_name` it pins `lan_encryption_mode = 2` (video/audio AES-GCM is otherwise cleartext on the LAN path - sunshine's default never offers video encryption; our moonlight always opts in, so mandatory rejects nothing) and `origin_web_ui_allowed = pc` (web UI answers localhost only). Verify encryption on a live session with `rg "Video encryption enabled" ~/Library/Logs/sunshine.log`.

One-time per machine, not declarable: Sunshine's Screen Recording grant (TCC), Sunshine web-UI creds (`sunshine --creds`), moonlight pairing (`sudo -u moonlight env QT_QPA_PLATFORM=offscreen moonlight pair <mac> --pin NNNN`, approve via `POST /api/pin` on the web UI; sunshine only runs while the cast is on, so pair mid-cast). DeskPad needs no grants: its preview capture is patched out at build (hosts/macbook/deskpad-no-preview.patch), which also kills macOS 26's recurring legacy-capture nag, and the supervisor launches it hidden (`open -gj`). It keeps a stable machine-local signing identity so LaunchServices and any future grant survive rebuilds.
