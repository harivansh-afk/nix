...
`claude-purple/` follows the same entry + lazy `core/` split. The cozybox theme keeps `accent`/`toolTitle` coral because omp colors tool titles, header descriptions, grep's per-file result headers, the tool dot, and the working loader all through the one `accent` token; the extension patches the Theme prototype (and `DEFAULT_SHIMMER_PALETTE`) so only the tool dots, the search dots, and the loader spinner/shimmer crest render claude-purple, read live from `getColorHex("statusLinePath")` (the prompting-bar path shares that lane in `lib/theme.nix`).
