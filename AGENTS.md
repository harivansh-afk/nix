...
The four proactive loops (`x-life-scan`, `hn-life-scan`, `dep-release-watch`, `finance-anomaly-watch`) are Hermes cron jobs, owned by `modules/services/hermes-loops.nix`. The split is deliberate: data acquisition stays deterministic (the scanner binaries - `x-feed-scan` drives Playwright against a saved X session, `hn-feed-scan` hits the HN Algolia API, `dep-release-scan` diffs GitHub releases against last-seen state in `/var/lib/loops`, `finance-anomaly-scan` computes anomaly candidates from the staged finance notes), judgment goes to a model, and delivery is always the agent over photon - so every ping is a resumable session in `~/.hermes/state.db` that `session_search` finds.

For the three feed loops, judgment is the agent itself: a per-job gather shim injects the scanner's stdout into the cron prompt (empty stdout prints `{"wakeAgent": false}`, hermes' wake gate, so a quiet feed costs no LLM run), and the `feed-triage` skill does the grounding, judging, KB note under `staging/loops/<job>/`, and the ping or `[SILENT]`.
...
Sandbox interplay to keep in mind: cron gather/relay scripts execute inside `hermes-gateway.service`, so anything they write must be in that unit's `ReadWritePaths` - currently `$HOME`, `staging/loops` (KB notes + the finance relay marker), `staging/saved` (read-it-later), and `/var/lib/loops` (dep-release last-seen state). Inspect the jobs with `hermes cron list`; trigger one with `hermes cron trigger <name>`; the judge with `sudo systemctl start finance-anomaly-judge`.
