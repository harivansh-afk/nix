...
- On spark: `kb-search "query"` is hybrid chunk search over the KB; `kb-graph resolve|neighbors|connect|source "<entity>"` walks the graph. `pasteimg` pulls the image in his Mac clipboard onto this host and prints a PNG path.
