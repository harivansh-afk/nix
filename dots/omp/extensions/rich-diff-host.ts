/**
 * rich-diff-host: diffs.nvim-style rendering for omp edit/apply_patch diffs.
 *
 * This module holds every value-import of the host package and is loaded
 * lazily by rich-diff.ts on session_start. It must NOT be imported statically
 * from the entry and must NOT be symlinked into ~/.omp/agent/extensions:
 * evaluating a host-package import triggers omp's bundled-registry cascade
 * (~850ms, measured with PI_TIMING) and would land inside loadExtensions on
 * every startup. Loaded at runtime instead, the relative import is served by
 * omp's permanent extension-graph onLoad hook, which re-reads this file from
 * disk and rewrites the host specifiers exactly like at load time - so the
 * cascade runs once, after the TUI is up.
 *
 ...
 * failure fall back to the stock renderer. Edits to this file need an omp
 * restart (the entry's ?mtime cache-buster does not reach runtime imports).
 */
import {
	editToolRenderer,
	getLanguageFromPath,
...
/** The mutable slot this extension patches on the host's edit tool renderer. */
...
export function install(): void {
	...
	// Guard above proved the shape; renderResult's real signature is not
	// exported, RenderResultFn mirrors it structurally.
