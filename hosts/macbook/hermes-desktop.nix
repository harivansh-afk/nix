...
# stub that fetches install.sh, whose "Build desktop app" stage runs `npm run
# pack` on the machine and installs a local agent beside it. The Electron app
# is built here from upstream's own nix package against the `minimal` agent
# instead, which carries none of the ML chain (torch, onnxruntime,
# faster-whisper) that `full` pulls in, and rebuilds only when the
# hermes-agent input bumps.
#
