...
file or affecting its other links. Deactivations survive service restarts.

The **Shares** link on the right of the header opens a table of active links,
with file paths, creation and expiration times, and copy/open/deactivate actions. It
...
The index records shares and durable deactivation markers. Public reads validate
signed tokens and check an in-memory set of deactivations; they do not read the
index. Keep this state directory to preserve deactivations across restarts.
Downloads already in progress cannot be recalled. Editing a file, including
replacing it atomically at the same path, updates what an active link serves.
