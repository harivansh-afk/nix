...
let
  ...
  aerospaceConfig = tomlFormat.generate "aerospace.toml" settings;
  ...
  aerospacePlist = ''
    ...
    <dict>
      <key>Label</key>
      <string>org.aerospace.aerospace</string>
...
lib.mkIf hostConfig.isDarwin {
  ...
  files.".config/aerospace/aerospace.toml".source = aerospaceConfig;
  files."Library/LaunchAgents/org.aerospace.aerospace.plist" = {
    text = aerospacePlist;
    force = true;
  };

  activationLines = ''
    plist="$HOME/Library/LaunchAgents/org.aerospace.aerospace.plist"
    if [ -f "$plist" ]; then
