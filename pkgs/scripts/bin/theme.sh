...
link_mode_assets() {
  ...
  mkdir -p "@STATE_DIR@" "@FZF_DIR@" "@GHOSTTY_DIR@" "@LAZYGIT_DIR@" "@LEAF_THEMES_DIR@" "@BTOP_THEMES_DIR@" "@GIT_THEME_DIR@" "@WALLPAPER_DIR@"
  printf '%s\n' "$mode" >"@STATE_FILE@"
  ...
  ln -sfn "$THEME_LEAF_TARGET" "@LEAF_CURRENT_FILE@"
  ln -sfn "$THEME_BTOP_TARGET" "@BTOP_CURRENT_FILE@"
